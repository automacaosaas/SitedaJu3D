...
// Email-first flow: verification precedes account creation; grants are one-use.
const emailFirst = createDemoAuth({now:()=>now,makeCode:()=>String(++serial)});
await assert.rejects(emailFirst.completeRegistration(person), /Confirme seu e-mail/);
let step = await emailFirst.begin({email:person.email});
assert.equal(step.purpose,'access');
await assert.rejects(emailFirst.completeRegistration(person), /Confirme seu e-mail/);
assert.equal((await emailFirst.verify({code:step.demoCode})).registrationAllowed,true);
const firstUser = await emailFirst.completeRegistration({...person,marketingOptIn:false});
assert.equal(firstUser.marketingOptIn,false);
await assert.rejects(emailFirst.completeRegistration(person), /Confirme seu e-mail/);
assert.equal((await emailFirst.login(person)).email,'teste@example.com');
now += 30000;
step = await emailFirst.begin({email:person.email});
assert.equal(step.purpose,'access','existing and new addresses have the same preverification flow');
assert.equal((await emailFirst.verify({code:step.demoCode})).user.email,'teste@example.com');
await assert.rejects(emailFirst.verify({code:step.demoCode}), /novo código/);
now += 30000;
step = await emailFirst.begin({email:'optin@example.com'});
await emailFirst.verify({code:step.demoCode});
assert.equal((await emailFirst.completeRegistration({...person,marketingOptIn:true})).marketingOptIn,true);
now += 30000;
step = await emailFirst.begin({email:'cancelled@example.com'});
await emailFirst.verify({code:step.demoCode}); emailFirst.cancel();
await assert.rejects(emailFirst.completeRegistration(person), /Confirme seu e-mail/);
await assert.rejects(emailFirst.begin({email:'cancelled@example.com'}), /30 segundos/,'cancel cannot bypass cooldown');
now += 30000;
step = await emailFirst.begin({email:'expired@example.com'});
await emailFirst.verify({code:step.demoCode}); now += 600000;
await assert.rejects(emailFirst.completeRegistration(person), /Confirme seu e-mail/);
console.log('PASS: email-first signup, verified account/password access, explicit consent, one-use/expired/cancelled registration grants and cooldown.');
