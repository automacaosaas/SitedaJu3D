...
function itemCard(item, selected) {
  ...
  return `<article class="cart-product ${selected ? 'is-selected' : 'is-unselected'}" aria-label="${esc(item.title)}">
    ...
    <div class="cart-product-art"><img src="assets/${esc(product.image)}" width="1024" height="1024" alt="${esc(item.title)} — imagem nas cores originais">${editButton(item, true)}</div>
