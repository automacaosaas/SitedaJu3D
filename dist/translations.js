// Portuguese source copy → English / Spanish. Brand and product names stay unchanged.
export const translations = Object.fromEntries(`
Para verificar sua conta nesta prévia, crie uma conta ou solicite um novo código. O envio real de e-mail ainda não está conectado.|To verify your account in this preview, create an account or request a new code. Real email delivery is not connected yet.|Para verificar tu cuenta en esta vista previa, crea una cuenta o solicita un nuevo código. El envío real de correo aún no está conectado.
← Voltar à vitrine|← Back to showcase|← Volver a la vitrina
CATÁLOGO JU|JU CATALOG|CATÁLOGO JU
Produtos feitos para deixar a consulta mais lúdica, colorida e acolhedora.|Products made to make appointments more playful, colorful and welcoming.|Productos para hacer la consulta más lúdica, colorida y acogedora.
Categorias de produtos|Product categories|Categorías de productos
Salvar e voltar ao carrinho|Save and return to cart|Guardar y volver al carrito
Salvo|Saved|Guardado
Peça adicionada!|Item added!|¡Pieza añadida!
Ver carrinho →|View cart →|Ver carrito →
Adicionado|Added|Añadido
Não foi possível preparar a compra. Verifique o armazenamento do navegador.|Unable to prepare your purchase. Check browser storage.|No se pudo preparar la compra. Comprueba el almacenamiento del navegador.
Imagem de apresentação • cores originais.|Presentation image • original colors.|Imagen de presentación • colores originales.
Arraste para girar · Prévia 3D ilustrativa, aguardando os modelos finais.|Drag to rotate · Illustrative 3D preview, awaiting final models.|Arrastra para girar · Vista previa 3D ilustrativa, a la espera de los modelos finales.
Preparando sua prévia 3D…|Preparing your 3D preview…|Preparando tu vista previa 3D…
A prévia 3D não está disponível neste navegador. Você pode continuar escolhendo as cores e consultar a imagem do produto.|3D preview is unavailable in this browser. You can still choose colors and view the product image.|La vista previa 3D no está disponible en este navegador. Puedes seguir eligiendo colores y consultar la imagen del producto.
Suas cores estão salvas neste navegador.|Your colors are saved in this browser.|Tus colores están guardados en este navegador.
Sua combinação está pronta nesta sessão.|Your combination is ready in this session.|Tu combinación está lista en esta sesión.
Cores originais restauradas para este produto.|Original colors restored for this product.|Colores originales restaurados para este producto.
Combinação copiada!|Combination copied!|¡Combinación copiada!
Não foi possível copiar automaticamente. As cores estão listadas acima.|Could not copy automatically. The colors are listed above.|No se pudo copiar automáticamente. Los colores están indicados arriba.
Pausar|Pause|Pausar
Pausar giro automático|Pause automatic rotation|Pausar giro automático
Coleção 3D|3D collection|Colección 3D
Pedido confirmado · demonstração|Order confirmed · demo|Pedido confirmado · demostración
Pedido criado|Order created|Pedido creado
Aguardando pagamento|Awaiting payment|Esperando el pago
Pagamento confirmado|Payment confirmed|Pago confirmado
Em preparação|In preparation|En preparación
Pronto para envio|Ready to ship|Listo para enviar
Andamento do pedido|Order progress|Estado del pedido
O tempo passou.|Time is up.|El tiempo pasó.
Suas escolhas ficaram.|Your choices are saved.|Tus elecciones siguen aquí.
Falta só|Just|Solo falta
um pequeno passo.|one small step.|un pequeño paso.
Pagamento de demonstração. Nenhum valor será movimentado.|Demo payment. No money will be transferred.|Pago de demostración. No se moverá dinero.
Código expirado|Code expired|Código caducado
Gere um novo código.|Generate a new code.|Genera un nuevo código.
Pague do seu jeito, com Pix.|Pay your way with Pix.|Paga a tu manera con Pix.
Seu pedido e suas cores continuam aqui.|Your order and colors are still here.|Tu pedido y tus colores siguen aquí.
No celular, copie o código. Em outro dispositivo, você usaria o QR Code.|On mobile, copy the code. On another device, you would use the QR code.|En el móvil, copia el código. En otro dispositivo, usarías el código QR.
DEMONSTRAÇÃO|DEMONSTRATION|DEMOSTRACIÓN
Imagem ilustrativa • não permite pagamentos|Illustrative image • cannot be used for payments|Imagen ilustrativa • no permite pagos
Válido por|Valid for|Válido durante
Pix copia e cola|Pix copy and paste|Pix copia y pega
demonstrativo|demo|demostrativo
Copiar código|Copy code|Copiar código
Gerar novo código de demonstração|Generate a new demo code|Generar nuevo código de demostración
Já realizei o pagamento · simular|I have paid · simulate|Ya he pagado · simular
Testar outro cenário|Test another scenario|Probar otro escenario
Simular expiração do Pix|Simulate Pix expiry|Simular caducidad de Pix
Cartão · demonstração|Card · demo|Tarjeta · demostración
Seu cartão, em boas mãos.|Your card is in good hands.|Tu tarjeta, en buenas manos.
Na versão final, o intermediador de pagamentos cuidará dos dados do seu cartão.|In the final version, the payment provider will handle your card details.|En la versión final, el proveedor de pagos se encargará de los datos de tu tarjeta.
PRÉVIA DE PAGAMENTO|PAYMENT PREVIEW|VISTA PREVIA DEL PAGO
Você não precisa digitar número, validade ou código de segurança para testar esta etapa.|You do not need to enter a card number, expiry date or security code to test this step.|No necesitas introducir número, fecha de caducidad ni código de seguridad para probar esta etapa.
Simular aprovação no cartão|Simulate card approval|Simular aprobación de tarjeta
Simular cartão recusado|Simulate declined card|Simular tarjeta rechazada
← Alterar dados ou pagamento|← Change details or payment|← Cambiar datos o pago
WhatsApp da Ju · em breve|Ju’s WhatsApp · coming soon|WhatsApp de Ju · próximamente
O contato será habilitado quando o número da loja for definido.|Contact will be enabled once the store’s number is set.|El contacto se habilitará cuando se defina el número de la tienda.
Suas cores.|Your colors.|Tus colores.
Pagamento aprovado na demonstração. Nenhuma cobrança ou produção foi iniciada.|Payment approved in the demo. No charge or production has started.|Pago aprobado en la demostración. No se ha iniciado ningún cobro ni producción.
Seu pedido ganhou vida.|Your order has come to life.|Tu pedido cobró vida.
Na loja final, a confirmação chega por aqui e a Ju recebe todos os detalhes para preparar suas peças.|In the final store, confirmation appears here and Ju receives all the details to prepare your pieces.|En la tienda final, la confirmación aparece aquí y Ju recibe todos los detalles para preparar tus piezas.
Produção estimada|Estimated production|Producción estimada
Copiar resumo para conversar com a Ju|Copy summary to talk with Ju|Copiar resumen para hablar con Ju
Continuar explorando|Keep exploring|Seguir explorando
O Pix de demonstração expirou. Gere um novo código para continuar.|The demo Pix expired. Generate a new code to continue.|El Pix de demostración caducó. Genera un nuevo código para continuar.
Código demonstrativo copiado. Ele não permite pagamentos.|Demo code copied. It cannot be used for payments.|Código de demostración copiado. No permite pagos.
Selecione e copie o código demonstrativo.|Select and copy the demo code.|Selecciona y copia el código de demostración.
Novo código de demonstração criado.|New demo code created.|Nuevo código de demostración creado.
Cartão recusado na simulação. Tente novamente ou escolha Pix; suas escolhas estão preservadas.|Card declined in the simulation. Try again or choose Pix; your choices are saved.|Tarjeta rechazada en la simulación. Inténtalo de nuevo o elige Pix; tus elecciones están guardadas.
Confirmando pagamento de demonstração…|Confirming demo payment…|Confirmando pago de demostración…
Processando a simulação.|Processing the simulation.|Procesando la simulación.
Pagamento confirmado na demonstração. Nenhum valor foi cobrado.|Payment confirmed in the demo. Nothing was charged.|Pago confirmado en la demostración. No se cobró ningún importe.
Resumo copiado.|Summary copied.|Resumen copiado.
Não foi possível copiar automaticamente. As escolhas estão no resumo ao lado.|Could not copy automatically. Your choices are in the summary alongside.|No se pudo copiar automáticamente. Tus elecciones están en el resumen al lado.
O carrinho foi alterado em outra aba. Confira os itens antes de continuar.|The cart changed in another tab. Check the items before continuing.|El carrito cambió en otra pestaña. Revisa los artículos antes de continuar.
Etapas da compra|Checkout steps|Pasos de compra
Meios de pagamento em demonstração|Demo payment methods|Medios de pago de demostración
Meios de pagamento|Payment methods|Medios de pago
Coleção de três produtos|Three-product collection|Colección de tres productos
Nossos três produtos|Our three products|Nuestros tres productos
Borboletoscópio verde-menta com detalhes amarelos sobre uma pilastra branca|Mint-green Borboletoscópio with yellow details on a white pedestal|Borboletoscópio verde menta con detalles amarillos sobre un pedestal blanco
Dinossauroscópio azul com espinhos verde-menta sobre uma pilastra branca|Blue Dinossauroscópio with mint-green spikes on a white pedestal|Dinossauroscópio azul con cresta verde menta sobre un pedestal blanco
Aviãoscopia sobre pilastra branca, com 16 aberturas numeradas e rasgo retangular para a haste da régua|Aviãoscopia on a white pedestal, with 16 numbered openings and a rectangular slot for the lens rack handle|Aviãoscopia sobre un pedestal blanco, con 16 aberturas numeradas y una ranura rectangular para el mango de la regla
Início|Home|Inicio
Produtos|Products|Productos
Sobre a Ju|About Ju|Sobre Ju
Contato|Contact|Contacto
Carrinho|Cart|Carrito
Entrega|Delivery|Entrega
Pagamento|Payment|Pago
Meu perfil|My profile|Mi perfil
Minha conta|My account|Mi cuenta
Entrar ou cadastrar|Sign in or register|Iniciar sesión o registrarse
Meus pedidos|My orders|Mis pedidos
Sair|Sign out|Salir
Um cantinho só seu.|A space just for you.|Un espacio solo para ti.
Navegação principal|Main navigation|Navegación principal
Navegação móvel|Mobile navigation|Navegación móvil
Compras e conta|Shopping and account|Compras y cuenta
Abrir menu|Open menu|Abrir menú
Fechar menu|Close menu|Cerrar menú
Menu principal|Main menu|Menú principal
PROTÓTIPO PARA AVALIAÇÃO|PREVIEW PROTOTYPE|PROTOTIPO DE EVALUACIÓN
Valores ilustrativos · sem cobranças|Sample prices · no charges|Precios ilustrativos · sin cobros
UM TOQUE DE IMAGINAÇÃO NO SEU CONSULTÓRIO|A TOUCH OF IMAGINATION IN YOUR PRACTICE|UN TOQUE DE IMAGINACIÓN EN TU CONSULTORIO
Mais cor na consulta.|More color at every appointment.|Más color en cada consulta.
Mais encanto em cada olhar.|More wonder in every glance.|Más encanto en cada mirada.
Para retinoscópio|For retinoscope|Para retinoscopio
Para régua de grau|For lens rack|Para regla de lentes
Conhecer|Discover|Descubrir
NOSSA COLEÇÃO|OUR COLLECTION|NUESTRA COLECCIÓN
Pequenas peças.|Little pieces.|Pequeñas piezas.
Grandes encantos.|Great wonders.|Grandes encantos.
Escolha uma peça para personalizar ou adicione-a direto ao carrinho.|Choose a piece to customize or add it straight to your cart.|Elige una pieza para personalizar o añádela directamente al carrito.
Ver catálogo completo|View full catalog|Ver catálogo completo
Preços ilustrativos para validação da experiência de compra.|Sample prices for previewing the shopping experience.|Precios ilustrativos para probar la experiencia de compra.
feito com carinho, pela Ju.|made with love, by Ju.|hecho con cariño, por Ju.
Cor e criatividade em cada camada.|Color and creativity in every layer.|Color y creatividad en cada capa.
Oftalmologia|Ophthalmology|Oftalmología
Sensoriais|Sensory|Sensoriales
Categoria|Category|Categoría
Cores|Colors|Colores
Capa para retinoscópio|Retinoscope cover|Funda para retinoscopio
Avião magnético para régua de grau|Magnetic airplane for lens rack|Avión magnético para regla de lentes
Preço ilustrativo|Sample price|Precio ilustrativo
Personalize o seu|Customize yours|Personaliza el tuyo
PERSONALIZE O SEU!|CUSTOMIZE YOURS!|¡PERSONALIZA EL TUYO!
Ver produto anterior|Previous product|Producto anterior
Ver próximo produto|Next product|Siguiente producto
Escolher produto|Choose product|Elegir producto
Produtos de oftalmologia|Ophthalmology products|Productos de oftalmología
carrossel|carousel|carrusel
em breve|coming soon|próximamente
EM BREVE|COMING SOON|PRÓXIMAMENTE
Novas ideias sensoriais estão chegando.|New sensory ideas are on their way.|Llegan nuevas ideas sensoriales.
Esta coleção está sendo preparada.|This collection is being prepared.|Estamos preparando esta colección.
Ela vai ganhar forma com o mesmo cuidado e imaginação da coleção atual.|It will take shape with the same care and imagination as our current collection.|Tomará forma con el mismo cuidado e imaginación que la colección actual.
SUAS ESCOLHAS|YOUR CHOICES|TUS ELECCIONES
Seu carrinho.|Your cart.|Tu carrito.
Seu carrinho|Your cart|Tu carrito
Confira seus produtos antes de continuar.|Review your products before continuing.|Revisa tus productos antes de continuar.
Selecionar todos os produtos|Select all products|Seleccionar todos los productos
Remover selecionados|Remove selected|Eliminar seleccionados
Remover produtos selecionados|Remove selected products|Eliminar productos seleccionados
Editar cores|Edit colors|Editar colores
← Continuar escolhendo|← Continue shopping|← Seguir eligiendo
Resumo do pedido|Order summary|Resumen del pedido
Resumo da compra e finalização|Order summary and checkout|Resumen de compra y finalización
Subtotal|Subtotal|Subtotal
Total|Total|Total
(exemplo)|(example)|(ejemplo)
exemplo|example|ejemplo
Finalizar pedido|Checkout|Finalizar pedido
Ver resumo|View summary|Ver resumen
Ver resumo ↓|View summary ↓|Ver resumen ↓
Selecione uma peça|Select a piece|Selecciona una pieza
Selecione uma peça para continuar.|Select a piece to continue.|Selecciona una pieza para continuar.
Compra segura|Secure shopping|Compra segura
Seus dados protegidos|Your data is protected|Tus datos protegidos
Produção sob demanda|Made to order|Producción bajo pedido
5 a 7 dias úteis (exemplo)|5 to 7 business days (example)|5 a 7 días hábiles (ejemplo)
Cartão|Card|Tarjeta
Seu carrinho espera um pouco de cor.|Your cart is waiting for a little color.|Tu carrito espera un poco de color.
Escolha uma peça e crie a sua combinação.|Choose a piece and create your combination.|Elige una pieza y crea tu combinación.
Explorar os produtos|Explore products|Explorar productos
Produtos no carrinho|Products in cart|Productos en el carrito
UM PASSO MAIS PERTO|ONE STEP CLOSER|UN PASO MÁS CERCA
COMPRAR AGORA|BUY NOW|COMPRAR AHORA
Para onde vai|Where should we send|¿Adónde va
esse carinho?|this little joy?|este cariño?
Preencha os dados de entrega e escolha como prefere pagar.|Enter your delivery details and choose how to pay.|Completa los datos de entrega y elige cómo pagar.
Quem vai receber?|Who will receive it?|¿Quién lo recibirá?
Use dados fictícios neste protótipo. Eles não são enviados nem salvos pelo formulário.|Use fictional details in this prototype. The form does not send or save them.|Usa datos ficticios en este prototipo. El formulario no los envía ni los guarda.
Nome completo|Full name|Nombre completo
E-mail|Email|Correo electrónico
WhatsApp com DDD|WhatsApp with area code|WhatsApp con código de área
Endereço de entrega|Delivery address|Dirección de entrega
CEP|Brazilian postal code|Código postal de Brasil
Cidade|City|Ciudad
Rua ou avenida|Street or avenue|Calle o avenida
Número|Number|Número
Bairro|Neighborhood|Barrio
Complemento|Apartment, suite, etc.|Complemento
(opcional)|(optional)|(opcional)
Estado|State|Estado
Selecione|Select|Selecciona
Observações|Notes|Observaciones
Algo que a Ju precisa saber?|Anything Ju should know?|¿Algo que Ju deba saber?
Entrega no seu endereço|Delivery to your address|Entrega en tu dirección
Frete e prazo finais serão definidos na integração.|Final shipping costs and times will be set when integration is complete.|El envío y los plazos finales se definirán con la integración.
Como prefere pagar?|How would you like to pay?|¿Cómo prefieres pagar?
Forma de pagamento|Payment method|Forma de pago
Copia e cola ou QR Code|Copy and paste or QR code|Copia y pega o código QR
Pagamento com intermediador|Payment through a payment provider|Pago mediante un proveedor
Aqui você pode experimentar a jornada completa, sem informar dados de cartão.|Try the full experience without entering card details.|Prueba la experiencia completa sin introducir datos de tarjeta.
← Voltar ao carrinho|← Back to cart|← Volver al carrito
← Rever minha combinação|← Review my combination|← Revisar mi combinación
Continuar para pagamento|Continue to payment|Continuar al pago
CADA DETALHE, DO SEU JEITO|EVERY DETAIL, YOUR WAY|CADA DETALLE, A TU MANERA
Feito sob encomenda|Made to order|Hecho por encargo
Preços, frete e prazo são exemplos para avaliação. A entrega real será calculada antes do pagamento.|Prices, shipping and timing are examples for evaluation. Actual delivery costs will be calculated before payment.|Los precios, el envío y los plazos son ejemplos. La entrega real se calculará antes del pago.
Foto nas cores originais|Photo in original colors|Foto en colores originales
Sua combinação · prévia 3D|Your combination · 3D preview|Tu combinación · vista previa 3D
Peça removida do carrinho.|Item removed from cart.|Pieza eliminada del carrito.
Quantidade atualizada.|Quantity updated.|Cantidad actualizada.
Itens selecionados removidos.|Selected items removed.|Artículos seleccionados eliminados.
Preencha os dados de entrega, sem deixar campos em branco.|Complete the delivery details without leaving blank fields.|Completa los datos de entrega sin dejar campos vacíos.
Voltar à coleção|Back to collection|Volver a la colección
FEITO PARA ENCANTAR|MADE TO DELIGHT|HECHO PARA ENCANTAR
Ir para o formulário|Skip to form|Ir al formulario
SEU CANTINHO|YOUR SPACE|TU ESPACIO
Seu cantinho|Your space|Tu espacio
Que bom ter você por aqui.|So glad you are here.|Qué alegría tenerte aquí.
Que bom ter|So glad|Qué alegría
você por aqui.|you are here.|tenerte aquí.
Acompanhe seus pedidos e explore novas cores quando quiser.|Track your orders and explore new colors whenever you like.|Sigue tus pedidos y explora nuevos colores cuando quieras.
Sessão demonstrativa nesta aba|Demo session in this tab|Sesión de demostración en esta pestaña
Sair da minha conta|Sign out of my account|Salir de mi cuenta
BEM-VINDA DE VOLTA|WELCOME BACK|TE DAMOS LA BIENVENIDA
Entre para acompanhar seus pedidos e descobrir criações pensadas para a sua rotina.|Sign in to track your orders and discover creations designed for your daily routine.|Entra para seguir tus pedidos y descubrir creaciones pensadas para tu rutina.
Entrar|Sign in|Entrar
Criar conta|Create account|Crear cuenta
Seu e-mail|Your email|Tu correo electrónico
Senha|Password|Contraseña
Sua senha|Your password|Tu contraseña
Esqueci minha senha|Forgot my password|Olvidé mi contraseña
Entrar na minha conta|Sign in to my account|Entrar en mi cuenta
Chegando agora?|New here?|¿Acabas de llegar?
Crie seu cantinho|Create your space|Crea tu espacio
VAMOS NOS CONHECER?|LET’S GET TO KNOW YOU|VAMOS A CONOCERNOS
Seu toque.|Your touch.|Tu toque.
Seu cantinho.|Your space.|Tu espacio.
Crie sua conta para acompanhar cada escolha, da primeira cor à chegada da sua peça.|Create an account to follow every choice, from the first color to your piece arriving.|Crea tu cuenta para seguir cada elección, desde el primer color hasta la llegada de tu pieza.
Como podemos chamar você?|What should we call you?|¿Cómo te llamas?
Seu nome|Your name|Tu nombre
Crie uma senha|Create a password|Crea una contraseña
Pelo menos 8 caracteres|At least 8 characters|Al menos 8 caracteres
Na prévia, use uma senha fictícia, diferente das suas senhas pessoais.|Use a fictional password for this preview, different from your personal passwords.|Usa una contraseña ficticia para esta vista previa, distinta de tus contraseñas personales.
Criar minha conta|Create my account|Crear mi cuenta
Já tem uma conta?|Already have an account?|¿Ya tienes una cuenta?
A GENTE AJUDA VOCÊ|WE’RE HERE TO HELP|ESTAMOS PARA AYUDARTE
Esqueceu a senha?|Forgot your password?|¿Olvidaste tu contraseña?
Acontece! Informe seu e-mail para iniciar a recuperação com um código de seis números.|It happens! Enter your email to start recovery with a six-digit code.|¡Suele pasar! Introduce tu correo para recuperarla con un código de seis dígitos.
E-mail da sua conta|Account email|Correo de tu cuenta
Receber código|Get code|Recibir código
← Voltar para entrar|← Back to sign in|← Volver a entrar
Voltar para entrar|Back to sign in|Volver a entrar
SÓ MAIS UM PASSINHO|JUST ONE MORE STEP|SOLO UN PASO MÁS
Confira seu e-mail.|Check your email.|Revisa tu correo.
Na versão conectada, o código chegará a|Once connected, the code will be sent to|En la versión conectada, el código llegará a
. Nesta prévia, consulte o código de teste abaixo.|. In this preview, see the test code below.|. En esta vista previa, consulta el código de prueba a continuación.
Código de verificação|Verification code|Código de verificación
Digite os seis números. O código vale por 10 minutos.|Enter the six digits. The code is valid for 10 minutes.|Introduce los seis dígitos. El código es válido durante 10 minutos.
Verificar código|Verify code|Verificar código
Confirmar meu e-mail|Confirm my email|Confirmar mi correo
Não recebeu?|Didn’t receive it?|¿No lo recibiste?
Reenviar código|Resend code|Reenviar código
← Alterar e-mail|← Change email|← Cambiar correo
CÓDIGO CONFERIDO|CODE VERIFIED|CÓDIGO VERIFICADO
Um novo começo.|A fresh start.|Un nuevo comienzo.
Escolha uma nova senha para voltar ao seu cantinho.|Choose a new password to return to your space.|Elige una nueva contraseña para volver a tu espacio.
Nova senha|New password|Nueva contraseña
Confirme a nova senha|Confirm new password|Confirma la nueva contraseña
Digite a senha novamente|Enter password again|Introduce la contraseña de nuevo
Salvar nova senha|Save new password|Guardar nueva contraseña
TUDO PRONTO|ALL SET|TODO LISTO
Senha renovada.|Password updated.|Contraseña actualizada.
Sua senha de teste foi atualizada. Você já pode entrar de novo nesta página.|Your test password has been updated. You can sign in again on this page.|Tu contraseña de prueba se ha actualizado. Ya puedes volver a entrar en esta página.
Processando…|Processing…|Procesando…
As senhas precisam ser iguais.|Passwords must match.|Las contraseñas deben coincidir.
Novo código de teste gerado.|New test code generated.|Nuevo código de prueba generado.
Mostrar senha|Show password|Mostrar contraseña
Ocultar senha|Hide password|Ocultar contraseña
Mostrar confirmação da senha|Show password confirmation|Mostrar confirmación de contraseña
Ocultar confirmação da senha|Hide password confirmation|Ocultar confirmación de contraseña
Acesso à conta|Account access|Acceso a la cuenta
Prévia interativa · sem envio de e-mail|Interactive preview · no emails sent|Vista previa interactiva · sin envío de correo
Use dados fictícios. As contas de teste e suas senhas derivadas ficam apenas na memória desta página e são apagadas ao recarregar. A sessão visual dura nesta aba; não dá acesso a dados reais.|Use fictional details. Test accounts and password hashes stay in this page’s memory and are cleared on reload. The visual session lasts in this tab and gives no access to real data.|Usa datos ficticios. Las cuentas de prueba y sus contraseñas derivadas quedan en la memoria de esta página y se borran al recargar. La sesión visual dura en esta pestaña y no da acceso a datos reales.
O código aparece aqui para testar o layout. A integração com o banco e o serviço de e-mail será conectada depois.|The code appears here to test the layout. Database and email integration will be connected later.|El código aparece aquí para probar el diseño. La integración con la base de datos y el correo se conectará después.
Código de teste — não enviado por e-mail|Test code — not sent by email|Código de prueba — no enviado por correo
Use apenas para testar esta prévia.|Use only to test this preview.|Úsalo solo para probar esta vista previa.
CADA ESCOLHA CONTA|EVERY CHOICE MATTERS|CADA ELECCIÓN CUENTA
Meus pedidos.|My orders.|Mis pedidos.
Acompanhe os pedidos demonstrativos feitos nesta aba.|Track demo orders placed in this tab.|Sigue los pedidos de demostración realizados en esta pestaña.
Pagamento simulado · nenhuma cobrança|Simulated payment · no charges|Pago simulado · sin cobros
Total com entrega demonstrativa|Total including demo delivery|Total con entrega de demostración
Produtos · entrega no resumo original|Products · delivery in original summary|Productos · entrega en el resumen original
Seu primeiro encanto|Your first little wonder|Tu primer encanto
está por vir.|is on its way.|está por llegar.
Quando você finalizar um pedido de teste, ele aparecerá aqui.|Your completed test orders will appear here.|Cuando finalices un pedido de prueba, aparecerá aquí.
Conhecer os produtos|Discover products|Conocer los productos
← Voltar à minha conta|← Back to my account|← Volver a mi cuenta
CRIATIVIDADE E CUIDADO|CREATIVITY AND CARE|CREATIVIDAD Y CUIDADO
PARA UMA ROTINA MAIS LEVE|FOR A BRIGHTER DAILY ROUTINE|PARA UNA RUTINA MÁS LIGERA
Explore as criações da Ju ↗|Explore Ju’s creations ↗|Explora las creaciones de Ju ↗
Criatividade,|Creativity,|Creatividad,
carinho e|love and|cariño y
detalhes que|details that|detalles que
encantam|delight|encantan
O 3D nas suas|3D at your|El 3D en tus
consultas|appointments|consultas
e em muitos|and in many|y en muchos
momentos|moments|momentos
SOBRE A JU|ABOUT JU|SOBRE JU
Uma história feita com carinho.|A story made with love.|Una historia hecha con cariño.
Estamos preparando este espaço para você conhecer a Ju e a história por trás de cada peça.|We are preparing this space so you can meet Ju and discover the story behind each piece.|Estamos preparando este espacio para que conozcas a Ju y la historia detrás de cada pieza.
Conteúdo em preparação|Content coming soon|Contenido en preparación
CONTATO|CONTACT|CONTACTO
Vamos conversar.|Let’s talk.|Hablemos.
Em breve, você encontrará aqui os canais oficiais para falar com a Ju sobre as peças e suas escolhas.|Soon you will find the official channels here to talk with Ju about her pieces and your choices.|Pronto encontrarás aquí los canales oficiales para hablar con Ju sobre las piezas y tus elecciones.
Fechar produto|Close product|Cerrar producto
Imagem|Image|Imagen
Prévia 3D|3D preview|Vista previa 3D
Girar|Rotate|Girar
Girar automaticamente|Rotate automatically|Girar automáticamente
Girar para a esquerda|Rotate left|Girar a la izquierda
Girar para a direita|Rotate right|Girar a la derecha
Aproximar|Zoom in|Acercar
Afastar|Zoom out|Alejar
Voltar à vista inicial|Reset view|Volver a la vista inicial
Tipo de visualização|View type|Tipo de vista
Controles do modelo 3D|3D model controls|Controles del modelo 3D
Imagem de apresentação baseada na peça real.|Presentation image based on the real piece.|Imagen de presentación basada en la pieza real.
Um toque seu, em cada detalhe.|Your touch, in every detail.|Tu toque en cada detalle.
Personalizar cores|Customize colors|Personalizar colores
Do seu jeito.|Your way.|A tu manera.
Voltar|Back|Volver
1. Escolha a parte|1. Choose a part|1. Elige la parte
2. Escolha a cor|2. Choose a color|2. Elige el color
Parte do produto|Product part|Parte del producto
Paleta de cores|Color palette|Paleta de colores
Restaurar cores|Reset colors|Restaurar colores
Paleta inicial|Original palette|Paleta inicial
Cores ilustrativas; podem variar conforme a tela e o filamento.|Illustrative colors; may vary by screen and filament.|Colores ilustrativos; pueden variar según la pantalla y el filamento.
Resumo da personalização|Customization summary|Resumen de personalización
Sua combinação.|Your combination.|Tu combinación.
Copiar combinação|Copy combination|Copiar combinación
Continuar personalizando ↗|Continue customizing ↗|Seguir personalizando ↗
Feito do seu jeito. Confira as cores antes de continuar.|Made your way. Check the colors before continuing.|Hecho a tu manera. Revisa los colores antes de continuar.
Concluir personalização|Finish customization|Finalizar personalización
Valor ilustrativo|Sample price|Precio ilustrativo
Sem cobrança nesta prévia|No charges in this preview|Sin cobros en esta vista previa
Comprar agora|Buy now|Comprar ahora
Adicionar ao carrinho|Add to cart|Añadir al carrito
Salvar alterações|Save changes|Guardar cambios
Verde-menta|Mint green|Verde menta
Azul-céu|Sky blue|Azul cielo
Azul-royal|Royal blue|Azul real
Rosa Ju|Ju pink|Rosa Ju
Lilás|Lilac|Lila
Amarelo|Yellow|Amarillo
Vermelho|Red|Rojo
Laranja|Orange|Naranja
Branco|White|Blanco
Preto|Black|Negro
Corpo|Body|Cuerpo
Detalhes das asas|Wing details|Detalles de las alas
Contorno, asas e antenas|Outline, wings and antennae|Contorno, alas y antenas
Parte interna e bolinhas|Inner section and dots|Parte interior y puntos
O rostinho e os olhos mantêm as cores originais.|The face and eyes keep their original colors.|La carita y los ojos mantienen sus colores originales.
Cabeça e corpo do dinossauro|Dinosaur head and body|Cabeza y cuerpo del dinosaurio
Crista e bolinhas|Spikes and dots|Cresta y puntos
A mesma cor nas duas partes|The same color for both parts|El mismo color en ambas partes
Os olhos permanecem sempre pretos.|The eyes always stay black.|Los ojos siempre son negros.
Fuselagem, asas e cauda|Fuselage, wings and tail|Fuselaje, alas y cola
Estrelas e topo|Stars and top|Estrellas y parte superior
A mesma cor nos dois detalhes|The same color for both details|El mismo color en ambos detalles
Motores|Engines|Motores
As duas peças sobre as asas|The two pieces on the wings|Las dos piezas sobre las alas
As janelas da cabine, lentes e aros mantêm as cores originais. A numeração acompanha cada abertura.|Cabin windows, lenses and rims keep their original colors. Each opening has its own number.|Las ventanas de la cabina, las lentes y los aros mantienen sus colores originales. Cada abertura tiene su número.
Uma borboleta para levar cor e imaginação à consulta. Feita em impressão 3D, com o espaço de encaixe do retinoscópio livre.|A butterfly to bring color and imagination to every appointment. 3D printed, with an open slot for the retinoscope.|Una mariposa para llevar color e imaginación a la consulta. Impresa en 3D, con el espacio para encajar el retinoscopio libre.
Um dinossauro simpático para acompanhar cada olhar. Capa impressa em 3D, com abertura para encaixar no retinoscópio.|A friendly dinosaur for every glance. A 3D-printed cover with an opening for the retinoscope.|Un dinosaurio simpático para acompañar cada mirada. Funda impresa en 3D con abertura para encajar el retinoscopio.
Um convite para a imaginação decolar. As 16 aberturas da régua lembram janelas de avião, com os graus identificados ao lado. O rasgo retangular na base acomoda a haste plana da régua. Apresentação ilustrativa com a régua encaixada.|An invitation to let imagination take flight. The rack’s 16 openings resemble airplane windows, with lens powers labeled alongside. The rectangular slot at the base holds the rack’s flat handle. Illustrative view with the rack inserted.|Una invitación a dejar volar la imaginación. Las 16 aberturas de la regla recuerdan a ventanas de avión, con las graduaciones indicadas al lado. La ranura rectangular de la base aloja el mango plano de la regla. Presentación ilustrativa con la regla encajada.
Use uma senha com 8 a 128 caracteres.|Use a password with 8 to 128 characters.|Usa una contraseña de 8 a 128 caracteres.
Aguarde 30 segundos antes de solicitar outro código.|Wait 30 seconds before requesting another code.|Espera 30 segundos antes de solicitar otro código.
Preencha seu nome e um e-mail válido.|Enter your name and a valid email.|Introduce tu nombre y un correo válido.
Este e-mail já foi cadastrado nesta prévia. Entre na sua conta.|This email is already registered in this preview. Sign in to your account.|Este correo ya está registrado en esta vista previa. Entra en tu cuenta.
E-mail ou senha não conferem nesta prévia. Crie uma conta de teste nesta página.|Email or password does not match in this preview. Create a test account on this page.|El correo o la contraseña no coinciden en esta vista previa. Crea una cuenta de prueba en esta página.
Informe um e-mail válido.|Enter a valid email.|Introduce un correo válido.
Solicite um novo código.|Request a new code.|Solicita un nuevo código.
O código expirou. Solicite um novo código.|The code expired. Request a new code.|El código ha caducado. Solicita uno nuevo.
Limite de tentativas atingido. Solicite outro código.|Attempt limit reached. Request another code.|Se alcanzó el límite de intentos. Solicita otro código.
O código não confere. Verifique os seis números.|The code does not match. Check the six digits.|El código no coincide. Revisa los seis dígitos.
Verifique um novo código para redefinir a senha.|Verify a new code to reset your password.|Verifica un nuevo código para restablecer la contraseña.
Não existe uma conta de teste nesta página. Comece por Criar conta.|There is no test account on this page. Start with Create account.|No hay una cuenta de prueba en esta página. Empieza por Crear cuenta.
Ju imprime pra mim — início|Ju imprime pra mim — home|Ju imprime pra mim — inicio
Ju, imprime pra mim? Criatividade em 3D|Ju, imprime pra mim? Creativity in 3D|Ju, imprime pra mim? Creatividad en 3D
CRIATIVIDADE EM 3D|CREATIVITY IN 3D|CREATIVIDAD EN 3D
Mais cor na consulta. Mais encanto em cada olhar.|More color at every appointment. More wonder in every glance.|Más color en cada consulta. Más encanto en cada mirada.
Escolha sua cor|Choose your color|Elige tu color
Produto anterior|Previous product|Producto anterior
Próximo produto|Next product|Siguiente producto
© 2026 Ju, imprime pra mim? Todos os direitos reservados.|© 2026 Ju, imprime pra mim? All rights reserved.|© 2026 Ju, imprime pra mim? Todos los derechos reservados.
Instagram da Ju, imprime pra mim? (abre em uma nova aba)|Ju, imprime pra mim? on Instagram (opens in a new tab)|Ju, imprime pra mim? en Instagram (se abre en una pestaña nueva)
Voltar à coleção da Ju|Back to Ju’s collection|Volver a la colección de Ju
← Voltar à coleção|← Back to collection|← Volver a la colección
Uma escolha sua. Um mundo de cor.|Your choice. A world of color.|Tu elección. Un mundo de color.
Julia sorri com o rosto apoiado na mão, de jaleco branco e roupa roxa, segurando uma borboleta ilustrada.|Julia smiles with a hand under the chin, in a white coat and purple outfit, holding an illustrated butterfly.|Julia sonríe con la mano bajo la barbilla, con bata blanca y ropa morada, sosteniendo una mariposa ilustrada.
ENTRANDO|SIGNING IN|INICIANDO SESIÓN
Abrindo o seu cantinho.|Opening your space.|Abriendo tu espacio.
Conferindo os seus dados|Checking your details|Revisando tus datos
Preparando a sua conta|Preparing your account|Preparando tu cuenta
Tudo certo!|All set!|¡Todo listo!
CADASTRO EM ANDAMENTO|SIGN-UP IN PROGRESS|REGISTRO EN CURSO
Preparando o seu cantinho.|Getting your space ready.|Preparando tu espacio.
Criando o seu cadastro na loja da Ju|Creating your account at Ju’s shop|Creando tu cuenta en la tienda de Ju
Enviando o código para o seu e-mail|Sending the code to your email|Enviando el código a tu correo
Cadastro criado!|Account created!|¡Cuenta creada!
Código enviado!|Code sent!|¡Código enviado!
Código gerado!|Code generated!|¡Código generado!
RECUPERAÇÃO|RECOVERY|RECUPERACIÓN
Enviando o seu código.|Sending your code.|Enviando tu código.
Conferindo o seu e-mail|Checking your email|Revisando tu correo
CONFIRMAÇÃO|CONFIRMATION|CONFIRMACIÓN
Conferindo o seu código.|Checking your code.|Revisando tu código.
Confirmando o seu e-mail.|Confirming your email.|Confirmando tu correo.
Conferindo o código|Checking the code|Revisando el código
Finalizando o seu cadastro|Finishing your sign-up|Terminando tu registro
Cadastro confirmado!|Sign-up confirmed!|¡Registro confirmado!
Código conferido!|Code verified!|¡Código verificado!
Leva só alguns segundos.|It only takes a few seconds.|Solo tarda unos segundos.
Enviamos um código de seis números para|We sent a six-digit code to|Enviamos un código de seis dígitos a
. Abra o e-mail e toque no botão, ou digite o código abaixo.|. Open the email and tap the button, or type the code below.|. Abre el correo y toca el botón, o escribe el código abajo.
Enviamos um novo código para o seu e-mail.|We sent a new code to your email.|Enviamos un nuevo código a tu correo.
Para verificar sua conta, crie uma conta ou peça um novo código.|To verify your account, create an account or request a new code.|Para verificar tu cuenta, crea una cuenta o pide un nuevo código.
Prévia interativa · contas de teste|Interactive preview · test accounts|Vista previa interactiva · cuentas de prueba
O código de verificação é enviado por e-mail. Se o envio não estiver disponível, um código de teste aparece aqui. Guardar as contas depende da integração com o banco de dados, que será conectada depois.|The verification code is sent by email. If sending is unavailable, a test code appears here. Saving accounts depends on the database integration, which will be connected later.|El código de verificación se envía por correo. Si el envío no está disponible, aquí aparece un código de prueba. Guardar las cuentas depende de la integración con la base de datos, que se conectará después.
Sem conexão. Verifique sua internet e tente de novo.|No connection. Check your internet and try again.|Sin conexión. Revisa tu internet e inténtalo de nuevo.
Muitas tentativas. Aguarde um instante e tente de novo.|Too many attempts. Wait a moment and try again.|Demasiados intentos. Espera un momento e inténtalo de nuevo.
Não foi possível enviar o e-mail agora. Tente novamente em instantes.|We could not send the email right now. Please try again shortly.|No pudimos enviar el correo ahora. Inténtalo de nuevo en unos instantes.
Não foi possível conferir o código agora. Tente novamente em instantes.|We could not check the code right now. Please try again shortly.|No pudimos revisar el código ahora. Inténtalo de nuevo en unos instantes.
Este link não é mais válido. Entre ou crie sua conta para receber um novo código.|This link is no longer valid. Sign in or create your account to get a new code.|Este enlace ya no es válido. Inicia sesión o crea tu cuenta para recibir un nuevo código.
Seu pedido|Your order|Tu pedido
Coleção de produtos|Product collection|Colección de productos
UM CANTINHO SÓ SEU|A SPACE THAT IS YOURS|UN RINCÓN SOLO TUYO
Tudo começa|It all starts|Todo empieza
com seu e-mail.|with your email.|con tu correo.
Preparando seu acesso…|Preparing your access…|Preparando tu acceso…
Carregando produtos|Loading products|Cargando productos
Preparando um pouco de encanto…|Preparing a little enchantment…|Preparando un poco de encanto…
Ative o JavaScript para explorar e personalizar as peças.|Enable JavaScript to explore and personalize the pieces.|Activa JavaScript para explorar y personalizar las piezas.
Prévia: use dados fictícios. Quando o envio não estiver disponível, um código de teste aparece nesta página.|Preview: use fictional details. When sending is unavailable, a test code appears on this page.|Vista previa: usa datos ficticios. Cuando el envío no esté disponible, aparece un código de prueba en esta página.
Enviamos um código de seis números para o seu e-mail. Toque no botão da mensagem ou digite o código abaixo.|We sent a six-digit code to your email. Tap the button in the message or type the code below.|Enviamos un código de seis dígitos a tu correo. Toca el botón del mensaje o escribe el código abajo.
Enviando outro código…|Sending another code…|Enviando otro código…
Voltar à página anterior|Back to the previous page|Volver a la página anterior
Peça adicionada. Indo para o carrinho…|Item added. Taking you to your cart…|Pieza añadida. Te llevamos al carrito…
Cores salvas. Voltando ao carrinho…|Colors saved. Returning to your cart…|Colores guardados. Volviendo al carrito…
Entre ou crie sua conta para acompanhar cada detalhe das suas escolhas.|Sign in or create your account to follow every detail of your choices.|Inicia sesión o crea tu cuenta para seguir cada detalle de tus elecciones.
Continuar|Continue|Continuar
Seu e-mail, com cuidado.|Your email, handled with care.|Tu correo, con cuidado.
Para acessar seu perfil e acompanhar pedidos. Novidades e ofertas, só se você escolher.|To access your profile and track orders. News and offers only if you choose.|Para acceder a tu perfil y seguir tus pedidos. Novedades y ofertas, solo si tú lo eliges.
Um instante de cuidado.|A moment of care.|Un momento de cuidado.
Seu acesso,|Your access,|Tu acceso,
com cuidado.|with care.|con cuidado.
Alterar|Change|Cambiar
Código de 6 números|6-digit code|Código de 6 dígitos
O código vale por 10 minutos.|The code is valid for 10 minutes.|El código es válido durante 10 minutos.
Confirmar e continuar|Confirm and continue|Confirmar y continuar
Precisa de outro código?|Need another code?|¿Necesitas otro código?
Usar minha senha|Use my password|Usar mi contraseña
Use a senha que criou no seu primeiro cadastro.|Use the password you created when you first signed up.|Usa la contraseña que creaste en tu primer registro.
Digite sua senha|Enter your password|Escribe tu contraseña
Usar código de acesso|Use an access code|Usar un código de acceso
Conferindo seu código…|Checking your code…|Revisando tu código…
E-mail confirmado.|Email confirmed.|Correo confirmado.
E-MAIL CONFIRMADO|EMAIL CONFIRMED|CORREO CONFIRMADO
Vamos nos|Let’s get|Vamos a
conhecer?|to know each other?|conocernos?
Só mais dois detalhes para criar seu cantinho.|Just two more details to create your space.|Solo dos detalles más para crear tu espacio.
Crie sua senha|Create your password|Crea tu contraseña
Quero receber novidades e ofertas da Ju por e-mail.|I would like to receive news and offers from Ju by email.|Quiero recibir novedades y ofertas de Ju por correo.
Opcional. Você pode mudar de ideia.|Optional. You can change your mind.|Opcional. Puedes cambiar de opinión.
Criando sua conta de teste…|Creating your test account…|Creando tu cuenta de prueba…
Preparando seu cantinho…|Getting your space ready…|Preparando tu espacio…
Conta de teste criada!|Test account created!|¡Cuenta de prueba creada!
Conta criada!|Account created!|¡Cuenta creada!
Acesso de teste confirmado!|Test access confirmed!|¡Acceso de prueba confirmado!
Acesso confirmado!|Access confirmed!|¡Acceso confirmado!
Conferindo seu acesso…|Checking your access…|Revisando tu acceso…
Tudo pronto para continuar.|Everything is ready to continue.|Todo listo para continuar.
Suas escolhas e seus próximos encantos, bem pertinho.|Your choices and your next little wonders, close by.|Tus elecciones y tus próximos encantos, muy cerca.
E-mail confirmado na prévia|Email confirmed in the preview|Correo confirmado en la vista previa
Novidades por e-mail: você escolheu receber.|News by email: you chose to receive.|Novedades por correo: elegiste recibir.
Novidades por e-mail: não autorizadas.|News by email: not authorized.|Novedades por correo: no autorizadas.
O endereço é informado na etapa de entrega.|The address is provided at the delivery step.|La dirección se indica en el paso de entrega.
Voltar ao carrinho|Back to cart|Volver al carrito
Sair da conta|Sign out|Cerrar sesión
Pedidos demonstrativos feitos nesta aba.|Demo orders placed in this tab.|Pedidos de demostración hechos en esta pestaña.
Voltar à minha conta|Back to my account|Volver a mi cuenta
Digite o código de teste abaixo para experimentar a confirmação do e-mail.|Enter the test code below to try the email confirmation.|Escribe el código de prueba de abajo para probar la confirmación del correo.
Código de teste · não enviado|Test code · not sent|Código de prueba · no enviado
Para verificar sua conta, informe seu e-mail e solicite um novo código.|To verify your account, enter your email and request a new code.|Para verificar tu cuenta, introduce tu correo y solicita un nuevo código.
CÓDIGO CONFIRMADO|CODE CONFIRMED|CÓDIGO CONFIRMADO
Escolha uma nova senha para acessar seu cantinho.|Choose a new password to access your space.|Elige una nueva contraseña para acceder a tu espacio.
Não foi possível continuar. Tente novamente.|We could not continue. Please try again.|No pudimos continuar. Inténtalo de nuevo.
Preparando a recuperação de acesso…|Preparing account recovery…|Preparando la recuperación de acceso…
Atualizando sua senha de teste…|Updating your test password…|Actualizando tu contraseña de prueba…
Senha de teste atualizada!|Test password updated!|¡Contraseña de prueba actualizada!
Este link não é mais válido. Solicite um novo código.|This link is no longer valid. Request a new code.|Este enlace ya no es válido. Solicita un nuevo código.
Confirme seu e-mail com um novo código.|Confirm your email with a new code.|Confirma tu correo con un nuevo código.
Informe seu nome (até 100 caracteres).|Enter your name (up to 100 characters).|Introduce tu nombre (hasta 100 caracteres).
Imagem indisponível.|Image unavailable.|Imagen no disponible.
Imagem indisponível|Image unavailable|Imagen no disponible
Conheça as cores da peça.|See the piece’s colors.|Conoce los colores de la pieza.
use Personalize o seu para escolher as cores.|use Customize yours to choose the colors.|usa Personaliza el tuyo para elegir los colores.
Falar com a Ju no WhatsApp ↗|Talk to Ju on WhatsApp ↗|Hablar con Ju por WhatsApp ↗
Informe um WhatsApp com DDD, por exemplo (11) 99999-9999.|Enter a WhatsApp number with area code, for example (11) 99999-9999.|Introduce un WhatsApp con código de área, por ejemplo (11) 99999-9999.
Demonstração aprovada, mas não foi possível atualizar o carrinho neste navegador.|Demo approved, but the cart could not be updated in this browser.|Demostración aprobada, pero no se pudo actualizar el carrito en este navegador.
`.trim().split('\n').map(line => { const [pt, en, es] = line.split('|'); return [pt, [en, es]]; }));
